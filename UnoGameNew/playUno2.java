...
public class playUno2 extends JFrame {
	...
	private ArrayList<Card> handAreaCard = new ArrayList<Card>();
	private ArrayList<JLabel> handAreaLabel = new ArrayList<JLabel>();
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater( () -> new playUno2().startUp());
	}
	...
	private void iniforNextRound() {
		playOrder = 1;
		currentPlayerIndex = 0;
		colorChoice = Color.GRAY;
		currentPlayAreaStartPosX = 250;
		currentPlayAreaStartPosY = 280;
		handAreaCard.clear();
		handAreaLabel.clear();
		handAreaCounter = -1;
		jlabelNumber = 0;	
	...
	private void handAreaPlaceCard(Card card, Color color, Point origin, Container handPanel) {
		...
		handAreaLabel.add(++handAreaCounter, createCardImage(imageFile, origin));
		handAreaLabel.get(handAreaCounter).setPreferredSize(new Dimension(60, 80));
		handPanel.add(handAreaLabel.get(handAreaCounter));
		handAreaCard.add(handAreaCounter, card);
		parent = handAreaLabel.get(handAreaCounter).getParent();
		handAreaLabel.get(handAreaCounter).addMouseListener(new MouseAdapter() {
            @Override
	...
	private void removeCardFromHandArea(Card clickedcard) {
		parent.remove(handAreaLabel.get(handAreaCard.indexOf(clickedcard)));
        parent.revalidate();
        parent.repaint();
        playArea.repaint();
	}
  ...
  //Create and set up a card image label.
	...
	*/
	...
	public void takeAction() {
		...
		// skip turn
			...
			//handle skips (reverse acts like skip in 2 player game)
			nextPlayer();
		}
		nextPlayer();
	}
	...
	public void nextRound() {
		...
		if (!gameOver) {
			...
			if (points[0] > 500) {
				String message = "CONGRATULATIONS YOU WON THIS GAME !!!";
				JOptionPane.showMessageDialog(null,message);
				frame.dispatchEvent(new WindowEvent(frame, WindowEvent.WINDOW_CLOSING));
			}
			else {
					if (points[computerPlayerIndex()] > 500) {
						String message = "CONGRATULATIONS Computer Player " + computerPlayerIndex() + " YOU WON THIS GAME !!!";
						frame.dispatchEvent(new WindowEvent(frame, WindowEvent.WINDOW_CLOSING));
						JOptionPane.showMessageDialog(null,message);
						frame.dispatchEvent(new WindowEvent(frame, WindowEvent.WINDOW_CLOSING));
					}
	...
	*/
	private void getPlayerPoints() {	
		...
		JPanel pointsPanel = new JPanel(new GridLayout(0,1,0,8));
		
		//Create label for Human Player point
		String message1 = "<html> <b>Human Player: </b>";
		...
		//Create Label for computer player points
		for (int i = 1; i < numPlayers(); i++) {
			String message3 = "<html> <b>Computer Player ";
			String message4 = Integer.toString(i);
			String message5 = ": ";
			String message6 = String.valueOf(points[i]);
			String message7 = message3 + message4 + message5 + message6 + "</b></html>";
